...
export class MemberService {
  ...
  createMember(member: ICreateMember): Observable<ICreateMember> {
    return this.http.post<ICreateMember>(
      environment.API_URL + 'Member',
      member
    );
  }
  ...
  getMemberDetailByPhone(number: string): Observable<IMember> {
    return this.http.get<IMember>(
      `${environment.API_URL}Member/getByPhone?number=${number}`
    );
  }
}
